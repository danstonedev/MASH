...
export function FirmwareUpdateCard() {
  ...
  const handleScanNetworks = async () => {
    ...
    try {
      const { connectionManager } =
        await import("../../lib/connection/ConnectionManager");
      await connectionManager.sendCommand("SCAN_WIFI");
      // Response arrives via json-packet event and is handled by the store
      await new Promise((r) => setTimeout(r, 10000));
      setWifiError("Check WiFi scan results");
    } catch (e) {
  ...
  const handleSoftAPToggle = async () => {
    ...
    try {
      const { connectionManager } =
        await import("../../lib/connection/ConnectionManager");
      if (newMode) {
        await connectionManager.sendCommand("START_SOFTAP", {
          ssid: "IMU-Connect-Setup",
          password: "",
        });
        setIsSoftAPMode(true);
        useDeviceStore.getState().setWifiIP("192.168.4.1");
      } else {
        await connectionManager.sendCommand("STOP_SOFTAP");
  ...
  const handleConnectWiFi = async () => {
    ...
    try {
      ...
      await connectionManager.sendCommand("SET_WIFI", {
        ssid: wifiSSID,
        password: wifiPassword,
      });
      await new Promise((r) => setTimeout(r, 500));
      await connectionManager.sendCommand("CONNECT_WIFI");

      let retries = 15;
      while (retries > 0) {
        await new Promise((r) => setTimeout(r, 1000));
        await connectionManager.sendCommand("GET_WIFI_STATUS");
      ...
      setWifiError(
        error instanceof Error ? error.message : "WiFi setup failed",
      );
    } finally {
  ...
  const handleWiFiOTA = async () => {
    ...
    try {
      startUpdate("gateway");
      const { loadFirmwareFromFile, performWiFiOTA } =
        await import("../../lib/ota");
    ...
    <div className="space-y-4">
        ...
        {!wifiIP ? (
          <>
            ...
            <Button
              ...
              disabled={!wifiSSID || !isConnected || wifiConnecting}
            >
      ...
      <div className="p-3 bg-bg-elevated rounded-lg border border-border/50">
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            className={cn(
              ...
              isSoftAPMode
                ? "bg-accent"
                : "bg-bg-primary border border-text-tertiary",
            )}
          >
            <div
              className={cn(
        ...
        {isSoftAPMode && (
          <div className="mt-2 text-[10px] text-text-secondary p-2 bg-bg-primary rounded">
            ...
            <p className="font-mono bg-black/20 p-1 rounded">
              IMU-Connect-Setup
            </p>
        ...
        {softAPError && (
          <p className="text-[10px] text-red-400 mt-1">{softAPError}</p>
        )}
      </div>
      ...
      <div className="space-y-3">
        ...
        <Button
          ...
          variant="gradient"
          className="w-full"
          onClick={handleWiFiOTA}
          disabled={!selectedFile || !wifiIP || isUpdating}
        >
...
function StatusBadge({
  ...
  if (status === "checking") {
    return (
      <div className="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-bg-elevated text-text-secondary border border-border">
        CHECKING...
      </div>
    );
  }
  if (status === "error" || status === "not_found") {
    return (
      <div className="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-warning/10 text-warning border border-warning/20">
        STATUS UNKNOWN
      </div>
    );
  }
