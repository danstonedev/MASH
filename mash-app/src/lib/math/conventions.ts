 ...
 * - Serial 0x03 packet: Packed as (rad/s * 100), parsed back to rad/s
 ...
 */
export function firmwareToThreeQuat(
 ...
 */
export function threeQuatToFirmware(
  q: THREE.Quaternion,
 ...
 * @returns true if |q| ≈ 1
 */
export function isUnitQuaternion(
  q: THREE.Quaternion,
  tolerance = 1e-6,
): boolean {
 ...
 */
export function isValidRotationMatrix(
  R: THREE.Matrix3,
  tolerance = 1e-6,
): boolean {
  // Check R * R^T ≈ I
